...
def write_mask_to_folder(mask, t_mask, path: str, num_contours) -> None:
    # Use OS-agnostic basename to avoid writing outside output on Windows
    filename = os.path.basename(t_mask)
...
def main(args: argparse.Namespace) -> None:
    ...
    if not os.path.isdir(args.input):
        ...
        # Sort to ensure deterministic pairing by filename
        targets.sort(key=lambda p: os.path.basename(p).lower())
    ...
    if not os.path.isdir(args.mask_input):
        ...
        # Sort to align with input images by filename
        targets_mask.sort(key=lambda p: os.path.basename(p).lower())
