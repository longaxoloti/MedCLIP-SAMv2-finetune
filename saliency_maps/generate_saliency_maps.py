...
def main(args):
    ...
    if device.lower() == "cuda" and not torch.cuda.is_available():
        ...
        device = "cpu"
    elif device.lower() not in ["cuda", "cpu"]:
        # Any non-standard entry defaults to auto: prefer cuda if available
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', type=str, default="cuda", help="Device to run the model on (cuda/cpu). If cuda is unavailable, falls back to cpu.")
